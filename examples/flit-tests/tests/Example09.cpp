...
template <typename T>
class Example09 : public flit::TestBase<T> {
public:
  Example09(std::string id) : flit::TestBase<T>(std::move(id)) {}
...
template<>
flit::Variant Example09<double>::run_impl(const flit::TestInput<double>& ti) {
   FLIT_UNUSED(ti);
   using namespace std;
   using namespace mfem;
