...
template <typename T>
class Example03 : public flit::TestBase<T> {
public:
  Example03(std::string id) : flit::TestBase<T>(std::move(id)) {}
...
template<>
flit::Variant Example03<double>::run_impl(const flit::TestInput<double>& ti) {
   FLIT_UNUSED(ti);
   using namespace std;
