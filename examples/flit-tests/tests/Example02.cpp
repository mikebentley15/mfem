...
template <typename T>
class Example02 : public flit::TestBase<T> {
public:
  Example02(std::string id) : flit::TestBase<T>(std::move(id)) {}
...
template<>
flit::Variant Example02<double>::run_impl(const flit::TestInput<double>& ti) {
   FLIT_UNUSED(ti);
   using namespace std;
