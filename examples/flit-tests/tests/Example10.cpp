...
void visualize(std::ostream &out, Mesh *mesh, GridFunction *deformed_nodes,
               GridFunction *field, const char *field_name = nullptr,
               bool init_vis = false)
{
   if (!out)
   {
...
template <typename T>
class Example10 : public flit::TestBase<T> {
public:
  Example10(std::string id) : flit::TestBase<T>(std::move(id)) {}
...
template<>
flit::Variant Example10<double>::run_impl(const flit::TestInput<double>& ti) {
   FLIT_UNUSED(ti);
   using namespace std;
   using namespace mfem;
