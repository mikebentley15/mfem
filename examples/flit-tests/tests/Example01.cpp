...
template <typename T>
class Example01 : public flit::TestBase<T> {
public:
  Example01(std::string id) : flit::TestBase<T>(std::move(id)) {}
...
template<>
flit::Variant Example01<double>::run_impl(const flit::TestInput<double>& ti) {
   FLIT_UNUSED(ti);
   using namespace std;
